...
var BTCE = function(apiKey, secret, nonceGenerator) {
  ...
  self.makeRequest = function(method, params, callback) {
    ...
    request({ url: self.url, method: "POST", form: params, headers: headers, timeout:15000, strictSSL:false}, function(err, response, body) {
  ...
  self.makePublicApiRequest = function(pair, method, callback) {
    request({ url: self.publicApiUrl + pair + '/' + method, timeout:15000, strictSSL:false}, function(err, response, body) {
      if(err || response.statusCode !== 200) {
        return callback(new Error(err ? err : response.statusCode));
      }
